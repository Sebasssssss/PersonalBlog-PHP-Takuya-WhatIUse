<?php
	...
	$listEquipment = $objIndex->listEquipment();
...
<body class="bg-orange-50 dark:bg-zinc-900 text-zinc-900 dark:text-zinc-300 transition-colors duration-500">
  ...
  <section class="px-8 mx-auto max-w-3xl mt-20">
    ...
    ?>
      <a href="cardsData.php?action=loadCard&cards=<?=$equipment['id']?>" class="w-[345px] mb-4 text-center">
        <img src="http://localhost/What-install/src/backend/files/images/<?=$equipment['image1']?>" class="w-full border border-slate-300 dark:border-slate-700 rounded-xl h-44">
        <h6 class="text-xl mt-3 font-semibold font-mplus"><?=$equipment['cardName']?></h6>
        <h1 class="opacity-70"><?=$equipment['subtitle']?></h1>
      </a>
    <?php
    } 
    ?>
