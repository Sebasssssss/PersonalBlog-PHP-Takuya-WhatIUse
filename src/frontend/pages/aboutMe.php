...
<body class="bg-orange-50 dark:bg-zinc-900 text-zinc-900 dark:text-zinc-300 transition-colors duration-500">
  ...
  <main class="h-80 pt-[56px]">
    <div class="flex flex-col gap-4 font-['Montserrat'] justify-center text-center py-5 absolute w-full bg-cover h-80 drop-shadow-lg shadow-black overflow-hidden">
    ...
    ?>
      <img src="http://localhost/What-install/src/backend/files/images/<?=$profile['profilePicture']?>" class="rounded-full w-24 mx-auto z-20 border border-slate-300 dark:border-slate-700 opacity-90 h-24 object-cover">
      <h1 class="z-20 text-lg font-mplus"><?=$profile['name']?></h1>
    <?php
    }
  ?>
    </div>
  ...
  ?>
    ...
    ?>
      <a href="cardsData.php?action=loadCard&cards=<?=$cards['id']?>" class="w-[345px] mb-4 text-center">
