<?php
	...
	$objIndex = new subpages_model();

	$cardsData = $objIndex->listequipment();
...
<head>
  ...
  <link rel="icon" href="../images/pawda.png">
    ...
    <div class="max-w-3xl mx-auto relative h-11">
      <nav class="flex items-center gap-3">
        ...
        <div class="items-center gap-6 hidden md:inline-flex pt-0.5">
          <a href="https://github.com/Sebasssssss/" class="text-md">About</a>
          <a href="https://github.com/Sebasssssss/PersonalBlog-PHP-Takuya-WhatIUse" class="inline-flex items-center text-md gap-1">
...
<section class="px-8 mx-auto max-w-3xl pt-12">
	
<?PHP 
	if(isset($_GET['action']) && $_GET['action'] == "loadCard" && isset($_GET['cards']) && $_GET['cards'] != "" ){
		$objIndex->load($_GET['cards']);
?>
	<div class="grid grid-cols-1 mt-8 gap-4" id="<?=$cards['id']?>">
		<div href="pages/tools.php" class="w-full mb-4 text-center">
			<img src="http://localhost/What-install/src/backend/files/images/<?=$objIndex->getImage2()?>" class="w-max mb-8 border border-slate-300 dark:border-slate-700 rounded-xl">
      <div class="flex flex-col border-b">
        <h1 class="text-3xl text-left font-mplus"><?=$objIndex->getCardName()?></h1>
        <h1 class="text-left py-2 pb-4"><?=$objIndex->getSubtitle()?></h1>
      </div>
			<p class="text-left pt-4"> <?=$objIndex->getDescription()?></p>
		</div>
		<div href="pages/tools.php" class="w-full mb-4 text-center">
			<img src="http://localhost/What-install/src/backend/files/images/<?=$objIndex->getImage3()?>" class="w-max border border-slate-300 dark:border-slate-700 rounded-xl">
		</div>
	</div>
<?php
}
